...
class database:
    def __init__(self):
        ...
        firebase_admin.initialize_app(self.cred, {'databaseURL': self.database_url_path} )
    ...
    def __get_data(self, key, value, db_directory = '/'):
        self.ref = db.reference(db_directory)
        search_data = self.ref.order_by_child(key).equal_to(value).get()
    ...
    def get_username_password(self, key, value, db_directory = '/'):
        self.ref = db.reference(db_directory)
        val = self.__get_data(key, value, db_directory)
